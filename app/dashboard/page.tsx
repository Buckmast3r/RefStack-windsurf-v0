...
export default function DashboardPage() {
  ...
  // Calculate additional stats for public stack
  const publicStackCount = referrals.filter((ref) => ref.includeInPublicStack).length
  const publicStackPercentage = referrals.length > 0 ? ((publicStackCount / referrals.length) * 100).toFixed(0) : "0"

  // Handle adding a new referral
  const handleAddReferral = (newReferral) => {
    const maxOrder = Math.max(...referrals.map((r) => r.displayOrder || 0), 0)
  ...
  // Handle public stack inclusion toggle
  const handlePublicStackToggle = (referralId, checked) => {
    setReferrals(referrals.map((ref) => (ref.id === referralId ? { ...ref, includeInPublicStack: checked } : ref)))
  }

  // Handle drag and drop reordering
  const handleDragStart = (e, referral) => {
  ...
  const handleDragEnter = (e, referral) => {
    e.preventDefault()
    setDragOverItem(referral)
  }

  const handleDrop = (e, targetReferral) => {
    e.preventDefault()

    if (!draggedItem || draggedItem.id === targetReferral.id) {
      return
    }

    const draggedIndex = referrals.findIndex((r) => r.id === draggedItem.id)
    const targetIndex = referrals.findIndex((r) => r.id === targetReferral.id)

    const newReferrals = [...referrals]

    // Remove dragged item
    const [removed] = newReferrals.splice(draggedIndex, 1)

    // Insert at new position
    newReferrals.splice(targetIndex, 0, removed)

    // Update display orders
    const updatedReferrals = newReferrals.map((ref, index) => ({
      ...ref,
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-950 via-indigo-950 to-purple-950 text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatsCard title="In Public Stack" value={`${publicStackCount} (${publicStackPercentage}%)`} />
        </div>
        ...
        {sortField === "displayOrder" && (
          <div className="mb-4 p-3 bg-blue-900/20 border border-blue-800/30 rounded-lg">
            <p className="text-sm text-blue-200/80 flex items-center">
              <GripVertical className="h-4 w-4 mr-2 text-blue-400" />
              Drag and drop rows to reorder how referrals appear on your public stack
            </p>
          </div>
        )}
        ...
        <div className="relative mb-8">
          ...
          <div className="relative bg-blue-950/40 backdrop-blur-md border border-blue-800/30 rounded-3xl overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)]">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-blue-800/50">
                    {sortField === "displayOrder" && (
                      <th className="px-3 py-4 text-left text-sm font-medium text-blue-200 w-12">
                    ...
                    <th className="px-6 py-4 text-center text-sm font-medium text-blue-200">
                      <button className="flex items-center mx-auto" onClick={() => handleSort("includeInPublicStack")}>
                        Public Stack
                        <ArrowUpDown
                          className={`ml-2 h-4 w-4 ${sortField === "includeInPublicStack" ? "text-blue-400" : "text-blue-300/50"}`}
                        />
                      </button>
                    </th>
                    <th className="px-6 py-4 text-center text-sm font-medium text-blue-200">
                      <button className="flex items-center mx-auto" onClick={() => handleSort("displayOrder")}>
                        Order
                        <ArrowUpDown
                          className={`ml-2 h-4 w-4 ${sortField === "displayOrder" ? "text-blue-400" : "text-blue-300/50"}`}
                        />
                      </button>
                    </th>
                ...
                <tbody>
                  {filteredAndSortedReferrals.length > 0 ? (
                    filteredAndSortedReferrals.map((referral) => (
                      <tr
                        ...
                        className={`border-b border-blue-800/30 hover:bg-blue-900/20 transition-colors ${
                          dragOverItem?.id === referral.id ? "bg-blue-800/30" : ""
                        } ${draggedItem?.id === referral.id ? "opacity-50" : ""}`}
                        draggable={sortField === "displayOrder"}
                        onDragStart={(e) => handleDragStart(e, referral)}
                        onDragEnd={handleDragEnd}
                        onDragOver={handleDragOver}
                        onDragEnter={(e) => handleDragEnter(e, referral)}
                        onDrop={(e) => handleDrop(e, referral)}
                      >
                        {sortField === "displayOrder" && (
                          <td className="px-3 py-4 text-center">
                            <GripVertical className="h-4 w-4 text-blue-400 cursor-grab active:cursor-grabbing mx-auto" />
                          </td>
                        )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <Checkbox
                            checked={referral.includeInPublicStack}
                            onCheckedChange={(checked) => handlePublicStackToggle(referral.id, checked)}
                            className="data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600"
                          />
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <span className="text-sm text-blue-300">{referral.displayOrder}</span>
                        </td>
                    ...
                    <tr>
                      <td
                        colSpan={sortField === "displayOrder" ? 9 : 8}
                        className="px-6 py-12 text-center text-blue-200"
                      >
